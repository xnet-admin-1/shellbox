...
public class TerminalSession extends TerminalOutput {
   ...
   */
  /** Create emulator only — no subprocess. Used by RishTerminalSession. */
  protected void initializeEmulatorOnly(int columns, int rows) {
  ...
  /** Feed bytes from an external process into the terminal emulator. */
  public void feedToEmulator(byte[] data, int offset, int count) {
    mProcessToTerminalIOQueue.write(data, offset, count);
    mMainThreadHandler.sendEmptyMessage(1);
  }

  /** Kill the local PTY subprocess. */
  public void killLocalProcess() {
    if (mShellPid > 0) {
      try {
        android.system.Os.kill(mShellPid, android.system.OsConstants.SIGKILL);
